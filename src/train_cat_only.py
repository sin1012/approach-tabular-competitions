...
def train(fold):
   ...
   cat_feas = [i for i in df.columns if i not in ['kfold', 'y']]
   ...
   joblib.dump(
      model,
      os.path.join(config.model_output, f'xgb_fold{fold}_cat_only.bin')
   )

   ## save the columns used to fit the model
   joblib.dump(
      cat_feas,
      os.path.join(config.model_output, f'xgb_cols_fold{fold}_cat_only.pkl')
   )
   
   ## save the label encoder
   joblib.dump(
      encoder,
      os.path.join(config.model_output, f'xgb_encoder_fold{fold}_cat_only.pkl')
   )
