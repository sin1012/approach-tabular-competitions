...
def train(fold):
   ...
   joblib.dump(
      model,
      os.path.join(config.model_output, f'lgb_fold{fold}.bin')
   )

   ## save the columns used to fit the model
   joblib.dump(
      df_train.drop(['kfold', 'y'], axis=1).columns,
      os.path.join(config.model_output, f'lgb_cols_fold{fold}.pkl')
   )
   
   ## save the label encoder
   joblib.dump(
      encoder,
      os.path.join(config.model_output, f'lgb_encoder_fold{fold}.pkl')
   )
